            ...
            <template slot="always-show">
                ...
                <Col class="form-item" span="6">
                    ...
                    <Select v-model="search.trspType" clearable>
                        <Option v-for="item in transportType" :key="item.dictionaryKey" :value="item.dictionaryKey">
        ...
        >
            ...
            >
                <div v-if="!isAdd" class="edit-field">
                    <span class="name">附加任务编号：</span>{{ editField }}
                </div>
                ...
                <FormItem label="物流类型" prop="logisticsType">
                    <Select v-model="formItem.logisticsType" clearable>
                        <Option v-for="item in logisticsType" :key="item.dictionaryKey" :value="item.dictionaryKey">
                            {{ item.dictionaryValue }}
                        </Option>
                    </Select>
                </FormItem>
            ...
            >
                ...
                <template #relationData="{ row, index }">
                    <Select v-model="row.relationData" clearable @on-change="changeRowData(row, index, 'relationData')">
                        <Option v-for="item in selectedData" :key="item.dataCode" :value="item.dataCode" :disabled="row.dataCode === item.dataCode ? true : false">
                            {{ item.dataName }}
                        </Option>
                    </Select>
                </template>
                <template #isMore="{ row, index }">
                    <Select v-model="row.isMore" clearable @on-change="changeRowData(row, index, 'isMore')">
                ...
                <template #isEdit="{ row, index }">
                    <Select v-model="row.isEdit" clearable @on-change="changeRowData(row, index, 'isEdit')">
                        <Option v-for="item in yesOrNo" :key="item.dictionaryKey" :value="item.dictionaryKey">
                            {{ item.dictionaryValue }}
                        </Option>
                    </Select>
                ...
                <template #isShow="{ row, index }">
                    <Select v-model="row.isShow" clearable @on-change="changeRowData(row, index, 'isShow')">
                        <Option v-for="item in yesOrNo" :key="item.dictionaryKey" :value="item.dictionaryKey">
                            {{ item.dictionaryValue }}
                        </Option>
                    </Select>
                ...
                <template #rules="{ row, index }">
                    <Select v-model="row.rules" multiple clearable @on-change="changeRowData(row, index, 'rules')">
...
<script>
    import {
        ...
        apiDelAffixTask,
        apiUpdateAffixTask,
        apiAffixTaskQueryDetail
    } from '@/api/standard-mapper/additionalTask'
    import validateRules from '@/libs/validateRules'
    import { isRepeat } from '@/libs/utils'
    import standardMapperList from '@/mixins/standardMapperList'
    import { formItem, additionalTaskColummns as columns, itemColumns, previewColumns } from '../basics-task/data'
    export default {
        name: 'AdditionalTask',
        ...
        data () {
            return {
                ...
                // 操作员启用申请、管理员启用
                apiEnable: apiAffixTaskEnable,
                // 操作员停用申请、管理员停用
                apiDisable: apiAffixTaskDisable,
                // 后端需要的启用、停用字段
                startAndStopData: ['taskCode', 'version', 'useStatus', 'examineStatus'],

                // 审核的api，作为参数传到mixin
                // 启用同意
                apiEnableExamPass: apiAffixTaskEnableExamPass,
                // 启用不同意
                apiEnableExamFail: apiAffixTaskEnableExamFail,
                // 停用同意
                apiDisableExamPass: apiAffixTaskDisableExamPass,
                // 停用不同意
                apiDisableExamFail: apiAffixTaskDisableExamFail,

                // 删除的api，作为参数传到mixin
                apiDel: apiDelAffixTask,
        ...
        methods: {
            ...
            async edit (row) {
                ...
                apiAffixTaskQueryDetail(params).then(({ data }) => {
                    // 反显选中的数据项
                ...
                this.$refs.formItemData.validate((valid) => {
                    if (valid) {
                        ...
                        if (this.isAdd) {
                            ...
                            // 编辑保存
                            apiUpdateAffixTask({ data }).then(res => {
                                successCallback()
                            })
                        }
                    }
