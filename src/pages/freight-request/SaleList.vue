<template>
    <div class="freight-request">
        ...
        >
            ...
            <template slot="always-show">
                <div class="public-transit">
                    <Form ref="search" inline label-colon :model="search">
                        <Row>
                            <Col span="8">
                                <FormItem label="启运地" class="min-width">
                                    <adt-city-selector @on-change="handelDirStart" />
                                </FormItem>
                                <FormItem label="选择场所">
                                    <Select v-model="search.departurePort" filterable clearable>
                                        <Option v-for="item in departurePort" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                    </Select>
                                </FormItem>
                            </Col>
                            <Col span="8">
                                <FormItem label="中转地" class="min-width">
                                    <adt-city-selector @on-change="handelDirTrsspmPort" />
                                </FormItem>
                                <FormItem label="选择场所">
                                    <Select v-model="search.trsspmPort" filterable clearable>
                                        <Option v-for="item in trsspmPort" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                    </Select>
                                </FormItem>
                            </Col>
                            <Col span="8">
                                ...
                                <FormItem label="选择场所">
                                    <Select v-model="search.destinationPort" filterable clearable>
                                        <Option v-for="item in destinationPort" :value="item.value" :key="item.value">{{ item.label }}</Option>
                                    </Select>
                                </FormItem>
                            </Col>
                        </Row>
                        <Row>
                            ...
                            <Col  span="6">
                                <FormItem label="货物类型">
                                    <Select v-model="search.cargoType" clearable>
                            ...
                            <Col  span="6">
                                <FormItem label="企业名称">
                                    <Input type="text" v-model="search.enterpriseCode" />
                            </FormItem>
                            </Col>
                            <Col  span="6">
                                <FormItem label="创建人">
                                    <Input type="text" v-model="search.createBy" />
                                </FormItem>
                            </Col>
                        </Row>
                    </Form>
        ...
        <div class="btns-paginator">
            <!-- <AdtButton :allDelShow="true" :selectList="selectList"> -->
            <AdtButton>
        ...
        <div class="public-list">
            <Col v-for="(product, key) in listData" :key="key" class="public-component-card public-card-width" span="24">
                <adt-list-item :data="product" :showCheckbox="true" @checkData="checkData">
                    ...
                    <template slot="tab-operat">
                        ...
                        <Poptip placement="right-start" class="card-icon">
                            ...
                            <!-- 接受的要求状态显示 -->
                            <div class="card-poptip-font" slot="content" v-if="queryType === '1'">
                                <!-- <div v-if="product.freightReqStatus === '待销售处理'" @click="edit(product)">编辑要求</div> -->
                                <div  @click="detail(product)">查看详情</div>
                                <div v-if="product.freightReqStatus === '待销售处理'" @click="send(product)">发送要求</div>
                                <div v-if="product.freightReqStatus === '已同意方案'" @click="viewRecommendations(product)">查看推荐方案</div>
                                <div v-if="product.freightReqStatus === '已反馈方案' || product.freightReqStatus === '调度已反馈方案'" @click="ViewLogisticsSolution(product)">查看物流方案</div>
                            </div>
                            <!-- 录入的要求的状态显示 -->
                            <div class="card-poptip-font" slot="content" v-else>
                                <!-- <div v-if="product.freightReqStatus === '未发送' || product.freightReqStatus === '未同意方案' || product.freightReqStatus === '取消货运要求'" @click="edit(product)">编辑要求</div> -->
                                <div  @click="detailSelf(product)">查看详情</div>
                                <div v-if="product.freightReqStatus === '未发送' || product.freightReqStatus === '取消货运要求'" @click="send(product)">发送要求</div>
                                <div v-if="product.freightReqStatus === '待调度处理' || product.freightReqStatus === '调度已反馈方案' || product.freightReqStatus === '未同意方案' || product.freightReqStatus === '已同意方案'" @click="cancel(product)">取消要求</div>
                                <!-- <div v-if="product.freightReqStatus === '未发送'|| product.freightReqStatus === '取消货运要求'" @click="delete(product)">删除</div> -->
                                <div v-if="product.freightReqStatus === '已同意方案'" @click="viewRecommendations(product)">查看推荐方案</div>
                            </div>
                        </Poptip>
                    </template>
                            ...
                            <Collapse accordion>
                                <Panel name="1">
                                    ...
                                    <div slot="content">
                                        <Row
                                            v-show="index !== 0"
                                            v-for="(itemList, index) of product.freightRouteList"
                                            :key="index"
                                            class="content-row"
                                        >
                                            <Col span="22">
                    ...
                    <template slot="tab-bottom">
                        <div class="tab-bottom">
                            <Row>
                                <Col span="24">录入时间：{{ timestampFormat(product.createDatetime) }}</Col>
                            </Row>
                        </div>
                    </template>
                </adt-list-item>
...
<script>
    ...
    import { apiSaleQueryFreight, apiSendDispatch } from '@/api/freight'
    ...
    export default {
        ...
        methods: {
            ...
            detail (value) {
                // 已经反馈的显示推荐方案
                if (value.freightReqStatus === '调度已反馈方案') {
                    this.$router.push({
                        path: `/freight-request/sale-to-edit/${value.freightReqCode}`,
                        query: {
                            sign: 'show'
                        }
                    })
                } else {
                    this.$router.push(`/freight-request/sale-to-edit/${value.freightReqCode}`)
                }
            },
            // 自己录入的详情
            detailSelf (value) {
                ...
                if (value.freightReqStatus === '调度已反馈方案') {
                    ...
                    this.$router.push(`/freight-request/feedback-detail/${value.freightReqCode}`)
                }
            },
            // 查看物流方案
            ...
            // 新增货运要求
            addFreight () {
                this.$router.push(`/freight-request/sale-entry`)
            },
            // 折叠面板数据
            ...
            getData () {
                ...
                apiSaleQueryFreight({ data }).then(res => {
                    if (res.data == null) {
                        this.listData = []
                        this.total = 0
                        return false
                ...
                switch (index) {
                    case 0:
                        ...
                        this.auditStatusList.push({
                            dictionaryKey: 'PENDSALE',
                            dictionaryValue: '待销售处理'
                        },{
                            dictionaryKey: 'BKSM',
                            dictionaryValue: '已反馈方案'
                        })
                        this.getData()
                        break
                    case 1:
                        this.auditStatusList.forEach((value, index, array) => {
                            ...
                            if (value.dictionaryValue === '已反馈方案'){
                            array.splice(value, 1)
                            }
                        })
                        this.auditStatusList.unshift({
                            dictionaryKey: 'UNSENT',
                            dictionaryValue: '未发送'
                        })
                        this.statusValue = 'UNSENT'
                        this.queryType = '0'
                        this.getData()
                        break
                }
