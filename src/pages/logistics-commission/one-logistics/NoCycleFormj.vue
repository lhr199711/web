                ...
                <Row :gutter="20">
                    ...
                    <Col span="6">
                        ...
                        <!-- <FormItem label="委托周期：" prop="entrustPeriod">
                            <Input v-model="formModel.entrustPeriod">
                                <span slot="append">天</span>
                            </Input>
                        </FormItem>-->
                        <FormItem label="合同委托有效期：" prop="cargoTotal">
                            <DatePicker
                                type="datetimerange"
                        ...
                        <FormItem label="货物总量：" prop="cargoTotal">
                            <Input v-model="formModel.cargoTotal">
                                <span slot="append">kg</span>
                            </Input>
                        </FormItem>
                        <FormItem class="jz-meaningless" v-for="(item,index) in 1" :key="index">
                ...
                <Row :gutter="20">
                    <Col span="18">
                        ...
                        <div class="scroll-box">
                            <cargo-info @cargoInfoItem="cargoInfoItem">
                                <template slot="btns-end">
                                    <span>货物类型：</span>
                                    <span>
                                        <Select v-model="planForm.cargoType" filterable clearable>
                                            <Option
...
<script>
import { apiAddPeriodEntrustBasicInfo } from '@/api/logisticsCommission'
import validateRules from '@/libs/validateRules'
import moment from 'moment'
import CargoInfo from './CargoInfo1'
    ...
    data () {
        return {
            ...
            formItemRule: {
                ...
                bitian: [validateRules.required('')[0]],
                entrustPeriod: [validateRules.required('')[0]],
                cargoTotal: [validateRules.required('')[0]],
                connum: [
        ...
        _add () {
            ...
            let list = this.CargoInfoList.map((item) => {
                ...
                return {
                    ...
                    cargoVol: item.cargoVol, //货物体积
                    // cargoQty: +item.cargoQty, // 货物数量 数字
            ...
            let data = {
                ...
                transportationAskReq: {
                    ...
                    startPeriod: this.formModel.startTime, // 开始时间
                    untilPeriod: this.formModel.endTime, // 结束时间
                    cargoTotal: +this.formModel.cargoTotal || 0, // 货物总量
                    entrustPeriod: +this.formModel.entrustPeriod || 0, // 委托周期
                },
                ...
                vodUnit: 'm³', //体积单位
                weightUnit: 'kg', //重量单位
                isDisciplinarian: 1, //0规律/1非规律 数字
            }
            apiAddPeriodEntrustBasicInfo({ data })
                .then(({ data }) => {
                    this.$Message.success('新增成功')
                    this.$store.commit(
                        'logisticsCommission/commitLawCallback',
                        data
                    )
                    this.$emit('handelDisble', data)
                })
                .catch((res) => { })
        },
        _process_datetimerange () {
            //处理时间取值头尾111
            if (this.timeArr && this.timeArr.length) {
                this.formModel.startTime = moment(this.timeArr[0]).valueOf()
                this.formModel.endTime = moment(this.timeArr[1]).valueOf()
            }
        },
    },
}
